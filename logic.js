...
// Declaring variables to be used for input
let startX = 0;
let startY = 0;
...
// This code ensures that our application will wait until everything on the page is ready before using function to load the tiles
window.onload = function(){
	setGame();
}

// HandleSlide() function is responsible for the logic whenver arrow keys are being pressed.
...
// FilterZerp() function removes the zero to the board, to make merging possible.
function filterZero(row){
	return row.filter(num => num != 0);
}

// Slide() function is responsible for the merging of the tiles
function slide(row){
	row = filterZero(row);
...
// SlideLeft is resposible in sliding to the tiles fot the left and merging matching adjacent tiles with the help of Slide() function
function slideLeft(){
	for(let r=0; r<rows; r++){
		let row = board[r];
...
// SlideRight() function is responsible in sliding the tile to the left and merging matching adjacent tiles with the help of Slide()
function slideRight(){
    for(let r=0; r<rows; r++){
...
// Function hasEmplyTile() checks if there is vacant slot/emply tile in the board.
...
// SetTwo function is responsible for generating a file with number 2
function setTwo(){
...
// CheckWin() function checks if there is a tile that has 2048,4096, 8192, so that the application will congratulate the user/player.
function checkWin(){
	for(let r = 0; r<rows; r++){
		for(let c= 0; c<columns; c++){
...
// hasLost() checks if there is a vacant tile because if there is, the player is not yet lost
// Also checks if there are tiles that could be merge. Because if it's possible, the player is not yet lost.
function hasLost(){
	for(let r = 0; r<rows; r++){
		for(let c= 0; c<columns; c++){
...
// restartGame() funcion restart the game board and the score
...
function canMoveLeft(){
	...
	for(let r = 0; r<rows; r++){
		for(let c= 1; c<columns; c++){
			...
			if(board[r][c] !== 0){
				if(board[r][c-1] === 0 || board[r][-1] === board[r][c]){
					return true;
				}
			}
